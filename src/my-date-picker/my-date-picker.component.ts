...
export const MYDP_VALUE_ACCESSOR: any = {
    ...
    useExisting: forwardRef(() => ng2datepickernevent),
    multi: true
};

@Component({
    selector: "my-date-picker",
    exportAs: "ng2datepickernevent",
    styles: [myDpStyles],
    template: myDpTpl,
...
export class ng2datepickernevent implements OnChanges, ControlValueAccessor {
    @Input() options: any;
    @Input() locale: string;
    ...
    opts: IMyOptions = {
        ...
        alignSelectorRight: <boolean> true,
    ...
    getColor = function ( month: number, day: number, year: number ) {
	var bDateMatch = '';
	var greenDiv = document.getElementById("emotionGreen");
	var items = greenDiv.getElementsByTagName("li");
	for (var i = 0; i < items.length; ++i) {
		if ( items[i].innerText.trim() == month+'/'+day+'/'+year )
		{
			bDateMatch = '5px solid Green';
			return bDateMatch;
		}
	}
	var yellowDiv = document.getElementById("emotionYellow");
	var items = yellowDiv.getElementsByTagName("li");
	for (var i = 0; i < items.length; ++i) {
		if ( items[i].innerText.trim() == month+'/'+day+'/'+year )
		{
			bDateMatch = '5px solid Yellow';
			return bDateMatch;
		}
	}
	var redDiv = document.getElementById("emotionRed");
	var items = redDiv.getElementsByTagName("li");
	for (var i = 0; i < items.length; ++i) {
		if ( items[i].innerText.trim() == month+'/'+day+'/'+year )
		{
			bDateMatch = '5px solid Red';
			return bDateMatch;
		}
	}
	/*$('#emotionGreen li').each( function () {
		if ( $(this).text().trim() == month+'/'+day+'/'+year )
		{
			bDateMatch = '5px solid Green';
			return;
		}
	})
	$('#emotionYellow li').each( function () {
		if ( $(this).text().trim() == month+'/'+day+'/'+year )
		{
			bDateMatch = '5px solid Yellow';
			return;
